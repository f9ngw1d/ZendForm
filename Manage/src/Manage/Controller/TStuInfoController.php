...
//        }
        ...
        if ($stu_info != null) {
            ...
            if(empty($stu_info->target_college)){
                $target_college_name = '未填写';
            }else{
                $target_college_name = $this->getCollegeTable()->getCollege($stu_info->target_college)->college_name;
            }
            if(empty($stu_info->target_team)){
                $target_team_name = '未填写';
            }else{
                $target_team_name = $this->getTeamTable()->getTeam($stu_info->target_team)->team_name;
            }
            $value = array(
                ...
                'ranking' => $rank1 . '/' . $rank2,
                'target_college' => $target_college_name,//empty($this->getCollegeTable()->getCollege($stu_info->target_college)->college_name)?'无':$this->getCollegeTable()->getCollege($stu_info->target_college)->college_name,
                'target_team' => $target_team_name,//empty($this->getTeamTable()->getTeam($stu_info->target_team)->team_name) ? '未填写' : $this->getTeamTable()->getTeam($stu_info->target_team)->team_name,
            );
            //add for show things
...
//        $login_id = 14;
        ...
        //查状态
//        $status = $this->getStuStatusTable()->getCheck($uid);
//        if($status->status<5){
//            echo "<script> alert('抱歉，该生当前状态不能打印复试单！');window.location.href='/info';</script>";
//        }
...
<style>
body{
    background:url('/img/ruc_pdflogo.png') no-repeat;
...
<div align='center'><img src=\"/img/barcodetmp/1.png\" style='position: center;'></div>
    ...
    <tr>
            <td colspan=\"7\" style=\"font-weight: bold;\">6.考核与评价（由组长填写）</td>
        </tr>
    <tr>
        <td colspan=\"7\">
            <br>
    ...
    <tr>
        <td colspan='7' align='center'>加试成绩（不计入考核总成绩）</td>
    </tr>
    <tr>
        <td>科目一名称及成绩</td>
        <td colspan='3'></td>
    ...
    <tr>
        <td colspan=\"7\" style=\"font-weight: bold;\">7.学院意见</td>
    </tr>
    <tr>
...
//    }
    ...
    public function getStuStatusTable(){
        if (!$this->StuStatusTable) {
            $sm = $this->getServiceLocator();
            $this->StuStatusTable = $sm->get('College\Model\StuStatusTable');
        }
        return $this->StuStatusTable;
    }
}